...
class PanelConnection extends EventEmitter {
  ...
  /**
   * Inhibit (bypass) a zone.
   * @param {number} zone - Zone number.
   * @returns {Promise<void>}
   */
  async inhibitZone(zone) {
    this._requireConnected();
    return this._client.inhibitZone(zone);
  }

  /**
   * Un-inhibit (restore) a zone.
   * @param {number} zone - Zone number.
   * @returns {Promise<void>}
   */
  async uninhibitZone(zone) {
    this._requireConnected();
    return this._client.uninhibitZone(zone);
  }

  /**
   * Get the area numbers a zone belongs to, or null when unknown.
   * @param {number} zone - Zone number.
   * @returns {number[]|null}
   */
  getZoneAreas(zone) {
    return this._client ? this._client.getZoneAreas(zone) : null;
  }
