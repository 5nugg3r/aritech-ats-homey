 ...
 * indicator shown on the tile; `alarm_tamper` is always present. For motion and
 * contact zones we additionally expose `alarm_generic` ("In alarm") so an armed
 * trip is visible separately from mere activity.
 * @type {Record<string, string[]>}
 */
const TYPE_CAPS = {
  motion: ['alarm_motion', 'alarm_generic', 'alarm_tamper'],
  contact: ['alarm_contact', 'alarm_generic', 'alarm_tamper'],
  fire: ['alarm_fire', 'alarm_tamper'],
  generic: ['alarm_generic', 'alarm_tamper'],
};
...
function guessZoneType(name) {
  ...
  if (/deur|door|raam|window|contact|magneet|meterkast|garage|poort|hek|reed|luik|schuifpui|\bpui\b|slot/.test(n)) return 'contact';
  return 'motion';
}
